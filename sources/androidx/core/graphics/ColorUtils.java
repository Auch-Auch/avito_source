...
public final class ColorUtils {
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        return Color.rgb(c(i3, 0, 255), c(i2, 0, 255), c(i, 0, 255));
    }
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3) {
        ...
        return Color.rgb(c((int) Math.round((d4 > 0.0031308d ? (Math.pow(d4, 0.4166666666666667d) * 1.055d) - 0.055d : d4 * 12.92d) * 255.0d), 0, 255), c((int) Math.round((d5 > 0.0031308d ? (Math.pow(d5, 0.4166666666666667d) * 1.055d) - 0.055d : d5 * 12.92d) * 255.0d), 0, 255), c((int) Math.round((d6 > 0.0031308d ? (Math.pow(d6, 0.4166666666666667d) * 1.055d) - 0.055d : d6 * 12.92d) * 255.0d), 0, 255));
    }
    ...
    public static void XYZToLAB(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3, @NonNull double[] dArr) {
        if (dArr.length == 3) {
            ...
            double d6 = d(d3 / 108.883d);
            dArr[0] = Math.max(0.0d, (116.0d * d5) - 16.0d);
            dArr[1] = (d4 - d5) * 500.0d;
            dArr[2] = (d5 - d6) * 200.0d;
            return;
        }
