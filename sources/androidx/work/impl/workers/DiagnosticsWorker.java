...
public class DiagnosticsWorker extends Worker {
    public static final String g = Logger.tagWithPrefix("DiagnosticsWrkr");
    ...
    public static String a(@NonNull WorkNameDao workNameDao, @NonNull WorkTagDao workTagDao, @NonNull SystemIdInfoDao systemIdInfoDao, @NonNull List<WorkSpec> list) {
        ...
        for (WorkSpec workSpec : list) {
            ...
            sb.append(String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", workSpec.id, workSpec.workerClassName, num, workSpec.state.name(), TextUtils.join(",", workNameDao.getNamesForWorkSpecId(workSpec.id)), TextUtils.join(",", workTagDao.getTagsForWorkSpecId(workSpec.id))));
        }
        return sb.toString();
    }
    ...
    public ListenableWorker.Result doWork() {
        ...
        if (recentlyCompletedWork != null && !recentlyCompletedWork.isEmpty()) {
            ...
            Logger.get().info(str, a(workNameDao, workTagDao, systemIdInfoDao, recentlyCompletedWork), new Throwable[0]);
        }
        if (runningWork != null && !runningWork.isEmpty()) {
            Logger logger2 = Logger.get();
            String str2 = g;
            logger2.info(str2, "Running work:\n\n", new Throwable[0]);
            Logger.get().info(str2, a(workNameDao, workTagDao, systemIdInfoDao, runningWork), new Throwable[0]);
        }
        if (allEligibleWorkSpecsForScheduling != null && !allEligibleWorkSpecsForScheduling.isEmpty()) {
            Logger logger3 = Logger.get();
            String str3 = g;
            logger3.info(str3, "Enqueued work:\n\n", new Throwable[0]);
            Logger.get().info(str3, a(workNameDao, workTagDao, systemIdInfoDao, allEligibleWorkSpecsForScheduling), new Throwable[0]);
        }
