...
public final class zzme implements zzmb {
    ...
    static {
        ...
        zza = zzdf.zza("measurement.ad_id_cache_time", 10000L);
        zzb = zzdf.zza("measurement.config.cache_time", 86400000L);
        ...
        zzf = zzdf.zza("measurement.upload.debug_upload_interval", 1000L);
        zzg = zzdf.zza("measurement.lifetimevalue.max_currency_tracked", 4L);
        zzh = zzdf.zza("measurement.store.max_stored_events_per_app", 100000L);
        zzi = zzdf.zza("measurement.experiment.max_ids", 50L);
        zzj = zzdf.zza("measurement.audience.filter_result_max_count", 200L);
        zzk = zzdf.zza("measurement.alarm_manager.minimum_interval", DefaultLoadErrorHandlingPolicy.DEFAULT_TRACK_BLACKLIST_MS);
        zzl = zzdf.zza("measurement.upload.minimum_delay", 500L);
        zzm = zzdf.zza("measurement.monitoring.sample_period_millis", 86400000L);
        zzn = zzdf.zza("measurement.upload.realtime_upload_interval", 10000L);
        zzo = zzdf.zza("measurement.upload.refresh_blacklisted_config_interval", 604800000L);
        zzp = zzdf.zza("measurement.config.cache_time.service", ConstantsKt.DEFAULT_TIME_TO_MODIFY_MESSAGE_MS);
        zzq = zzdf.zza("measurement.service_client.idle_disconnect_millis", 5000L);
        zzr = zzdf.zza("measurement.log_tag.service", "FA-SVC");
        zzs = zzdf.zza("measurement.upload.stale_data_deletion_interval", 86400000L);
        zzt = zzdf.zza("measurement.upload.backoff_period", 43200000L);
        zzu = zzdf.zza("measurement.upload.initial_upload_delay_time", 15000L);
        zzv = zzdf.zza("measurement.upload.interval", ConstantsKt.DEFAULT_TIME_TO_MODIFY_MESSAGE_MS);
        zzw = zzdf.zza("measurement.upload.max_bundle_size", PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH);
        zzx = zzdf.zza("measurement.upload.max_bundles", 100L);
        zzy = zzdf.zza("measurement.upload.max_conversions_per_day", 500L);
        zzz = zzdf.zza("measurement.upload.max_error_events_per_day", 1000L);
        zzaa = zzdf.zza("measurement.upload.max_events_per_bundle", 1000L);
        zzab = zzdf.zza("measurement.upload.max_events_per_day", 100000L);
        zzac = zzdf.zza("measurement.upload.max_public_events_per_day", 50000L);
        zzad = zzdf.zza("measurement.upload.max_queue_time", 2419200000L);
        zzae = zzdf.zza("measurement.upload.max_realtime_events_per_day", 10L);
        zzaf = zzdf.zza("measurement.upload.max_batch_size", PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH);
        zzag = zzdf.zza("measurement.upload.retry_count", 6L);
        zzah = zzdf.zza("measurement.upload.retry_time", 1800000L);
        zzai = zzdf.zza("measurement.upload.url", "https://app-measurement.com/a");
        zzaj = zzdf.zza("measurement.upload.window_interval", ConstantsKt.DEFAULT_TIME_TO_MODIFY_MESSAGE_MS);
    }
