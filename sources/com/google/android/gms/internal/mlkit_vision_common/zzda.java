...
public abstract class zzda<MessageType extends zzda<MessageType, BuilderType>, BuilderType extends zzcz<MessageType, BuilderType>> implements zzfr {
    public int zza = 0;
    ...
    public static <T> void zza(Iterable<T> iterable, List<? super T> list) {
        ...
        if (iterable instanceof zzfc) {
            List<?> zzd = ((zzfc) iterable).zzd();
            zzfc zzfc = (zzfc) list;
            int size = list.size();
            for (Object obj : zzd) {
                if (obj == null) {
                    String o2 = a.o2(37, "Element at index ", zzfc.size() - size, " is null.");
                    int size2 = zzfc.size();
                    while (true) {
                        size2--;
                        if (size2 >= size) {
        ...
        } else if (iterable instanceof zzge) {
