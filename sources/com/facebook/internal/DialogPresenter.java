...
public class DialogPresenter {
    ...
    public static Uri a(DialogFeature dialogFeature) {
    ...
    public static NativeProtocol.ProtocolVersionQueryResult getProtocolVersionForNativeDialog(DialogFeature dialogFeature) {
        ...
        FetchedAppSettings.DialogFeatureConfig dialogFeatureConfig = FetchedAppSettings.getDialogFeatureConfig(applicationId, action, dialogFeature.name());
        return NativeProtocol.getLatestAvailableProtocolVersionForAction(action, dialogFeatureConfig != null ? dialogFeatureConfig.getVersionSpec() : new int[]{dialogFeature.getMinVersion()});
    }
    ...
    public static void setupAppCallForWebFallbackDialog(AppCall appCall, Bundle bundle, DialogFeature dialogFeature) {
        ...
        if (a != null) {
            ...
            if (queryParamsForPlatformActivityIntentWebFallback != null) {
                if (a.isRelative()) {
                    uri = Utility.buildUri(ServerProtocol.getDialogAuthority(), a.toString(), queryParamsForPlatformActivityIntentWebFallback);
                } else {
                    uri = Utility.buildUri(a.getAuthority(), a.getPath(), queryParamsForPlatformActivityIntentWebFallback);
                }
                Bundle bundle2 = new Bundle();
        ...
        throw new FacebookException(a.e3("Unable to fetch the Url for the DialogFeature : '", name, "'"));
    }
