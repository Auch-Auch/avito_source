...
public class GlGenericDrawer implements RendererCommon.GlDrawer {
    ...
    public static String createFragmentShaderString(String str, ShaderType shaderType) {
        ...
        if (shaderType == ShaderType.YUV) {
            a.n1(sb, "uniform sampler2D y_tex;\n", "uniform sampler2D u_tex;\n", "uniform sampler2D v_tex;\n", "vec4 sample(vec2 p) {\n");
            a.n1(sb, "  float y = texture2D(y_tex, p).r * 1.16438;\n", "  float u = texture2D(u_tex, p).r;\n", "  float v = texture2D(v_tex, p).r;\n", "  return vec4(y + 1.59603 * v - 0.874202,\n");
            a.n1(sb, "    y - 0.391762 * u - 0.812968 * v + 0.531668,\n", "    y + 2.01723 * u - 1.08563, 1);\n", "}\n", str);
        } else {
            a.m1(sb, "uniform ", shaderType == shaderType2 ? "samplerExternalOES" : "sampler2D", " tex;\n");
    ...
    private void prepareShader(ShaderType shaderType, float[] fArr, int i, int i2, int i3, int i4) {
        ...
        GLES20.glVertexAttribPointer(this.inPosLocation, 2, (int) R2.styleable.Motion_motionPathRotate, false, 0, (Buffer) FULL_RECTANGLE_BUFFER);
        GLES20.glEnableVertexAttribArray(this.inTcLocation);
        GLES20.glVertexAttribPointer(this.inTcLocation, 2, (int) R2.styleable.Motion_motionPathRotate, false, 0, (Buffer) FULL_RECTANGLE_TEXTURE_BUFFER);
